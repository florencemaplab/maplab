(function () {
  ...
  function safePublicationURL(url, doi = "", scopusId = "", eid = "") {
    ...
    const cleanScopus = String(scopusId || "")
      .replace(/^SCOPUS_ID:/i, "")
      .trim();

    if (cleanScopus) {
      return `https://www.scopus.com/inward/record.uri?partnerID=HzOxMe3b&scp=${encodeURIComponent(cleanScopus)}&origin=inward`;
    }

    const cleanEid = String(eid || "").trim();
    if (cleanEid.startsWith("2-s2.0-")) {
      return `https://www.scopus.com/inward/record.uri?partnerID=HzOxMe3b&scp=${encodeURIComponent(cleanEid.replace("2-s2.0-", ""))}&origin=inward`;
    }
...
function escapeHTML(value) {
  ...
  function injectScopusAnalyticsStyles() {
    ...
    style.textContent = `
      ...
      .scopus-analytics-personal .coauthor-card {
        display: none !important;
      }
  ...
  async function renderScopusAnalytics(prefix, slug, profile) {
    ...
    wrapper.className = "scopus-analytics scopus-analytics-personal";
    wrapper.innerHTML = `
      ...
      <div class="scopus-visuals scopus-visuals-single">
        <div class="word-cloud-card">
          ...
          <p class="scopus-note">Generated from publication titles indexed in Scopus. Abstract retrieval is disabled because the current Scopus API access does not provide stable abstract access.</p>
        </div>
      </div>
    `;
    ...
    // Safety cleanup for old cached/generated markup: individual pages must not show a coauthor network.
    panel.querySelectorAll(".coauthor-card").forEach((element) => element.remove());
  }

  
  function removeIndividualCoauthorCards() {
    document.querySelectorAll(".coauthor-card").forEach((element) => element.remove());
  }

async function init() {
    removeIndividualCoauthorCards();
